
import './App.css';

function App() {
  return (
    <div className="App">
      <header className="App-header">
        <h1>Hello, World</h1>
        <p>You can do this. Keep going. Dont give up!</p>
        <p>You dont give up.</p>
        <p>Youre going places!</p>
        <p>Keep going!</p>
        <p>You got this!</p>
        <p>I can do this!!!!</p>
        <h2>New Title!!</h2>
        <p>Hi!</p>
        <p>Hello!</p>
        <p>Keep going@</p>
        <l>New list</l>
        <p>You are great</p>
        <h2>New header</h2>
        <h3>New Header</h3>
        <l>New list</l>
        <h2>new header</h2>
        <p>new ine</p>
        <p>new line</p>
        <h3>Keep going</h3>
        <p>new line</p>
        <div>
          <p>New div</p>
        </div>
        <p>new line</p>
        <div>
          <p>new div</p>
        </div>
        <p>I don't give up
        </p>
        <p>Tomorrow is a new day</p>
        <p>I want to give up but im here`</p>
        <p>I feel better today than yesterday</p>
        <p>I feel better today than yesterday</p>
        <p>im feeling okay today</p>
        <p>Im feeling somewhat good toda</p>
        <p>Feeling okay today</p>
        <p>Im feeling hopeful today</p>
        <p>Im feeling pretty good today. Taking each day one step at a time.</p>
        <p>Feeling pretty tired today</p>
        <p>im feeling okay today </p>
        <p>Feeling okay today</p>
        <p>Feeling less motivated today</p>
        <p>Im tired of being fat. Im going to chanage for good.</p>
        <p>im feeling okay today</p>
        <p>im feeling motivated today</p>
        <p>Im feeling good today</p>
        <p>IM feeling okay today</p>
        <p>Im tired but still going to the gyn today</p>
        <p>Extremly tired today but still making it to the offce</p>
        <p>Im feeling okay today</p>
        <p>Im feeling pissed today</p>
        <p>Im feeking okay today </p>
        <p>Starfield comes out tonigth!</p>
        <p>im feeling okay today</p>
        <p>im feelikng okay today</p>
        <p>I can do this</p>
        <p>im feelking okay today</p>
        <p>im feeling hopeful today.</p>
        <p>Im feeling tired today</p>

      </header>
    </div>
  );
}

export default App;
